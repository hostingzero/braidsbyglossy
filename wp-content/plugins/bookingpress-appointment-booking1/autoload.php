...
if( !empty( $wp_version ) && version_compare( $wp_version, '5.3.0', '<' ) ) {
    ...
    $plugin_deactivation_note = sprintf( __( 'Current WordPress version does not meet minimum requirements for BookingPress. Please update the WordPress version to at least 5.3.0 and then try again. Click %s to continue', 'bookingpress-appointment-booking' ), '<a href="'. $plugin_deactivation_link.'">'.__('here', 'bookingpress-appointment-booking').'</a>' );

    wp_die( $plugin_deactivation_note );
