...
if (! class_exists('BookingPress') ) {
    class BookingPress Extends BookingPress_Core
    {
        ...
        function upgrade_data()
        {
            ...
            if (version_compare($bookingpress_old_version, '1.0.74', '<') ) {
        ...
        public function bookingpress_retrieve_off_days( $start_date = '', $number_of_days = 366, $selected_service = '', $selected_service_duration = '', $selected_staffmember = '' ){
            ...
            $bpa_default_daysoff_where = $wpdb->prepare( 'WHERE bookingpress_dayoff_date >= %s', $start_date .' 00:00:00' );
            ...
            foreach( $period as $dt ){
                ...
                if( in_array( strtolower( $weekday_name ), $default_off_days['off_days'] ) && ( empty( $exclude_days ) || ( !empty( $exclude_days ) && !in_array( $dt->format('Y-m-d H:i:s'), $exclude_days ) ) ) ){
